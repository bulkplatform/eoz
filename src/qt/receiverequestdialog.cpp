...
#include <config/eozi-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update()
{
    ...
    QString uri = GUIUtil::formatEoziURI(info);
    ...
    if(info.amount)
        html += "<b>"+tr("Amount")+"</b>: " + EoziUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount) + "<br>";
...
{
    GUIUtil::setClipboard(GUIUtil::formatEoziURI(info));
}
